...
def test_admin_can_assign_and_remove_moderator_role(client, test_settings):
    ...
    csrf_token = _extract_csrf_token(admin_users_page.text)
    ...
    response = client.post(
        f"/admin/users/{target.id}/role",
        data={"role": "user", "_csrf_token": csrf_token},
        follow_redirects=False,
    )
    assert response.status_code == 303
...
def test_admin_page_shows_moderator_controls_and_safe_fields(client, test_settings):
    ...
    assert "Сделать модератором" not in body
...
def test_non_admins_cannot_assign_moderators(client, test_settings):
    ...
    anonymous_response = client.post(
        f"/admin/users/{target.id}/role",
        data={"role": "moderator", "_csrf_token": login_csrf_token},
        follow_redirects=False,
    )
    ...
    user_cabinet_page = client.get("/cabinet")
    assert user_cabinet_page.status_code == 200
    user_csrf_token = _extract_csrf_token(user_cabinet_page.text)
    user_response = client.post(
        f"/admin/users/{target.id}/role",
        data={"role": "moderator", "_csrf_token": user_csrf_token},
    )
    ...
    moderator_cabinet_page = client.get("/cabinet")
    assert moderator_cabinet_page.status_code == 200
    moderator_csrf_token = _extract_csrf_token(moderator_cabinet_page.text)
    moderator_response = client.post(
        f"/admin/users/{target.id}/role",
        data={"role": "moderator", "_csrf_token": moderator_csrf_token},
    )
...
def test_admin_cannot_demote_admin_or_set_arbitrary_roles(client, test_settings):
    ...
    target = _create_verified_user(test_settings, "moderator-target-admin@example.com", "moderatortargetadmin")
    _login_as(client, test_settings, admin.email)
    admin_users_page = client.get("/admin/users")
    assert admin_users_page.status_code == 200
    csrf_token = _extract_csrf_token(admin_users_page.text)

    response = client.post(f"/admin/users/{admin.id}/role", data={"role": "user", "_csrf_token": csrf_token})
    assert response.status_code == 400
    assert _role_for_email(test_settings, admin.email) == "admin"

    arbitrary_response = client.post(
        f"/admin/users/{target.id}/role",
        data={"role": "owner", "_csrf_token": csrf_token},
    )
    assert arbitrary_response.status_code == 400
    assert "допустимую роль" in arbitrary_response.text.lower()
    assert _role_for_email(test_settings, target.email) == "user"
